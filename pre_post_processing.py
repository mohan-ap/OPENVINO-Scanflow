...
data_folder = "data"
data_folder_path = os.path.join(current_directory, data_folder)

character_dict_path = os.path.join(data_folder_path, "text/en_dict.txt")
...
## Postprocessing for chinese recognition model
character_key_path = os.path.join(data_folder_path, "text/ppocr_keys_v1.txt")


ch_postprocess_params = {
            'name': 'CTCLabelDecode',
            "character_type": "ch",
            'character_dict_path': character_key_path,
            "use_space_char": True
        }
                ...
                try:
                    ...
                    if is_remove_duplicate:
                        ...
                        if idx > 0 and text_index[batch_idx][idx - 1] == text_index[batch_idx][idx]:
                            continue
                    char_index = int(text_index[batch_idx][idx])
                    if char_index < 0 or char_index >= len(self.character):
                        continue
                    char_list.append(self.character[char_index])
